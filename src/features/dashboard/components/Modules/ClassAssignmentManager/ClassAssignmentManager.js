...
export function ClassAssignmentManager() {
    ...
    const filteredAssignments = useMemo(() => {
        return assignments.filter(assignment => {
            ...
            if (filters.weeklyClasses && assignment.schedule_type !== 'weekly')
                return false;
            // Class status filter
            if (filters.classStatus.length > 0 && assignment.class_status && !filters.classStatus.includes(assignment.class_status))
                return false;
            // Payment status filter
            if (filters.paymentStatus.length > 0 && assignment.payment_status && !filters.paymentStatus.includes(assignment.payment_status))
                return false;
    ...
    // Assignment actions
    const openClassDetails = (assignment) => {
