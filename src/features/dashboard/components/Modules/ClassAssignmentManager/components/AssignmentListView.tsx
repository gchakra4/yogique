        ...
        <div className="overflow-x-hidden">
            <div className="space-y-2">
                {groupedAssignments.map(group => {
                    ...
                    const parseLocalDate = (s?: string) => s ? new Date(s + 'T00:00:00') : null

                    const packageCount = group.assignments.find(a => (a as any).package && (a as any).package.class_count)?.package?.class_count
                        || (group.assignments.find(a => (a as any).total_classes) as any)?.total_classes

                    const looksLikeCrash = group.type === 'crash_course'
                        || group.groupInfo.pattern_description === 'Crash Course'
                        || !!packageCount
                        || group.groupInfo.class_type_name?.toLowerCase().includes('crash')

                    const inPeriod: typeof group.assignments = (group.type === 'monthly' && !looksLikeCrash)
                        ? group.assignments.filter(a => (a.date || '').startsWith(yyyyMm))
                        : group.assignments

                    const totalFromPackage = packageCount || undefined
                    const total = (looksLikeCrash && typeof totalFromPackage === 'number') ? totalFromPackage : inPeriod.length

                    let remaining = 0
                    if (looksLikeCrash && typeof totalFromPackage === 'number') {
                        const completedCount = group.assignments.filter(a => a.class_status === 'completed').length
                        remaining = Math.max(total - completedCount, 0)
                    } else {
                        const todayDateOnly = new Date(today.getFullYear(), today.getMonth(), today.getDate())
                        remaining = inPeriod.filter(a => {
                    ...
                    const visibleAssignments = (group.type === 'monthly' && !looksLikeCrash) ? inPeriod : group.assignments
