...
import { ConflictDetails, FormData, ValidationErrors } from '../types'
...
export const useFormHandler = ({
    ...
    const debouncedOnFormChange = useCallback(
        debounce((data: FormData) => onFormChange?.(data), debounceMs),
        [onFormChange, debounceMs]
    )
            ...
            case 'monthly':
                if (formData.start_date) {
                    if (selectedPackage?.class_count && selectedPackage?.validity_days) {
                        ...
                        calculatedEndDate = calculateCourseEndDate(
                            formData.start_date,
                            formData.course_duration_value,
                            formData.course_duration_unit
                        )

                        totalClasses = calculateTotalClassesForRecurrence(
                            formData.course_duration_value,
                            formData.course_duration_unit,
                            formData.monthly_assignment_method,
                            formData.weekly_days.length
                ...
                if (formData.start_date) {
                    if (selectedPackage?.class_count) {
                        ...
                        if (selectedPackage.validity_days) {
                            ...
                            calculatedEndDate = calculateCourseEndDate(
                                formData.start_date,
                                formData.course_duration_value || 2,
                                formData.course_duration_unit || 'weeks'
                            )
                ...
                if (formData.start_date) {
                    if (selectedPackage?.class_count) {
                        ...
                        if (selectedPackage.validity_days) {
                            ...
                            calculatedEndDate = calculateCourseEndDate(
                                formData.start_date,
                                formData.course_duration_value || 4,
                                formData.course_duration_unit || 'weeks'
                            )
    ...
    const handleInputChange = useCallback((field: string, value: any) => {
        setFormData(prev => {
            ...
            if (field === 'assignment_type') {
                ...
                newData.booking_type = ASSIGNMENT_BOOKING_MAP[value as keyof typeof ASSIGNMENT_BOOKING_MAP] || 'individual'
            }
    ...
    const handleTimeChange = useCallback((field: 'start_time' | 'end_time', time: string) => {
    ...
    const handleDurationChange = useCallback((durationMinutes: number) => {
...
function calculateValidityEndDate(startDate: string, validityDays: number): string {
    const endDate = new Date(startDate)
    endDate.setDate(endDate.getDate() + validityDays)
    return endDate.toISOString().split('T')[0]
}

function calculateTotalClassesForRecurrence(
...
function calculateCrashCourseInfo(formData: FormData) {
    if (formData.course_duration_value && formData.course_duration_unit) {
        ...
        if (formData.monthly_assignment_method === 'weekly_recurrence' && formData.weekly_days.length > 0) {
            ...
            totalClasses = calculateCourseClasses(
                formData.course_duration_value,
                formData.course_duration_unit,
...
function calculatePackageInfo(formData: FormData) {
    if (formData.course_duration_value && formData.course_duration_unit) {
        const calculatedEndDate = calculateCourseEndDate(
            formData.start_date,
            formData.course_duration_value,
        ...
        const totalClasses = calculateTotalClassesForRecurrence(
            formData.course_duration_value,
            formData.course_duration_unit,
            formData.monthly_assignment_method,
            formData.weekly_days.length
...
function validateByAssignmentType(formData: FormData): ValidationErrors {
    ...
    switch (formData.assignment_type) {
        ...
        case 'weekly':
            if (formData.monthly_assignment_method === 'weekly_recurrence') {
                ...
                if (!formData.course_duration_value) errors.course_duration_value = 'Duration is required'
                if (formData.course_duration_value <= 0) errors.course_duration_value = 'Duration must be greater than 0'
                if (!formData.day_of_week && formData.day_of_week !== 0) errors.day_of_week = 'Day of week is required'
            }
            break
        ...
        case 'crash_course':
            if (!formData.package_id) errors.package_id = 'Package is required'
            if (!formData.start_date) errors.start_date = 'Start date is required'
            if (!formData.class_frequency) errors.class_frequency = 'Class frequency is required'
