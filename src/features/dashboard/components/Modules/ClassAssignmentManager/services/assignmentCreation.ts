 ...
 */
const checkBookingAccessStatus = async (bookingIds: string[]): Promise<{ allowed: boolean; reason?: string; status: string }> => {
    if (!bookingIds || bookingIds.length === 0) {
...
export class AssignmentCreationService {
    ...
    private static async createMonthlyAssignment(formData: FormData, studentCount: number) {
        ...
        try {
            ...
            throw new Error(
                `Calendar Month Violation: ${error instanceof Error ? error.message : String(error)}\n\n` +
    ...
    private static async markBookingsAsRecurring(bookingIds: string[], billingAnchorDate: string | undefined, packageId?: string | null) {
        if (!bookingIds || bookingIds.length === 0) return

        const payload: any = { is_recurring: true }
        if (billingAnchorDate) payload.billing_cycle_anchor = billingAnchorDate
        if (packageId) payload.class_package_id = packageId

        // Update by booking_id (external code) rather than internal id
        for (const bookingCode of bookingIds) {
            if (!bookingCode || bookingCode.trim() === '') continue
            try {
                const { error } = await supabase
                    .from('bookings')
                    .update(payload)
    ...
    private static async generateFirstMonthInvoices(
        ...
        if (!bookingIds || bookingIds.length === 0 || !packageId) return
        ...
        try {
            ...
            for (const bookingCode of bookingIds) {
                ...
                try {
                    ...
                    const result = await createFirstMonthInvoice(
                        bookingCode.trim(),
                        booking.user_id,
                        startDate,
                        monthlyPrice,
    ...
    private static async generateWeeklyRecurrenceAssignments(formData: FormData, perClassAmount: number, calendarMonth: string) {
        const currentUserId = await getCurrentUserId()
        const assignments = []
    ...
    private static async generateManualCalendarAssignments(formData: FormData, perClassAmount: number, calendarMonth: string) {
        const currentUserId = await getCurrentUserId()
    ...
    private static async createCrashCourseAssignment(formData: FormData, packages: Package[], studentCount: number) {
        ...
        const assignments = []
        let crashCourseValidation: any = null // Store for later validation
        ...
        crashCourseValidation = await validateCrashCourseAssignment(
            formData.package_id,
            formData.start_date,
            crashCourseDates
        )
