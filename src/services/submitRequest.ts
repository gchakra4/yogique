...
export default async function submitRequest(opts: SubmitOpts = {}) {
  const runtime = typeof window !== 'undefined' ? (window as any).DEVTOOLS_CONFIG || {} : {}
  const funcUrl = runtime.SUBMIT_REQUEST_URL || (import.meta.env.VITE_SUBMIT_REQUEST_URL as string | undefined)
