...
function baseUrl() { return SUPABASE_URL.replace(/\/+$/g, '') }

serve(async (req) => {
  try {
    const auth = req.headers.get('authorization')
    if (!auth) return new Response(JSON.stringify({ error: 'missing auth' }), { status: 401 })
    ...
    if (!rolesResp.ok) return new Response(JSON.stringify({ error: 'failed to fetch roles' }), { status: 403 })
