...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch booking using service role to verify ownership
    const bookingUrl = `${SUPABASE_URL.replace(/\/+$/, '')}/rest/v1/bookings?select=id,booking_id,user_id&${body?.booking_id ? `booking_id=eq.${encodeURIComponent(body.booking_id)}` : `id=eq.${encodeURIComponent(body.id)}`}`
    const bookingRes = await fetch(bookingUrl, { headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` } })
    if (!bookingRes.ok) {
      const t = await bookingRes.text().catch(() => '')
      console.error('Failed to query booking', bookingRes.status, t)
