// CommonJS version: Scans workspace for references to public tables and reports match counts and files
// Usage: node tools/scan-table-references.cjs > docs/zero-reference-report.json

const fs = require('fs');
const path = require('path');

const ROOT = path.resolve(__dirname, '..');
// Scan only runtime code by default; docs and backups create false positives
const INCLUDE_DIRS = [
    'src',
    'supabase/functions'
];
    ...
    while (stack.length) {
        ...
        for (const e of entries) {
            ...
            if (e.isDirectory()) { stack.push(p); }
                ...
                if (FILE_EXTS.includes(path.extname(p))) {
                    out.push(p);
                }
            }
        }
